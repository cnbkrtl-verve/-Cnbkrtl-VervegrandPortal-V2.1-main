...
if 'authentication_status' not in st.session_state or not st.session_state[
        'authentication_status']:
    st.warning("Lütfen devam etmek için giriş yapın.")
    st.stop()
if 'shopify_status' not in st.session_state or st.session_state[
        'shopify_status'] != 'connected':
    st.error(
        "Shopify bağlantısı kurulu değil. Lütfen Ayarlar sayfasından bilgilerinizi kontrol edin.")
    st.stop()
...
with st.expander("🔍 Sipariş Filtreleme ve Arama", expanded=True):
    ...
    with date_cols[2]:
        sort_order = st.selectbox(
            "Sıralama", [
                "En Yeni", "En Eski", "Tutar (Yüksek-Düşük)", "Tutar (Düşük-Yüksek)"])

    # Alt sıra: Status filtreleri
    filter_cols = st.columns(4)
    with filter_cols[0]:
        financial_filter = st.selectbox(
            "Ödeme Durumu", [
                "Tümü", "PAID", "PENDING", "REFUNDED", "PARTIALLY_PAID"])
    with filter_cols[1]:
        fulfillment_filter = st.selectbox(
            "Kargo Durumu", [
                "Tümü", "FULFILLED", "UNFULFILLED", "PARTIALLY_FULFILLED"])
    with filter_cols[2]:
        customer_search = st.text_input(
            "Müşteri Ara", placeholder="İsim, email veya telefon")
    with filter_cols[3]:
        order_search = st.text_input(
            "Sipariş No", placeholder="#1001, #1002...")

    fetch_button = st.button(
        "📥 Shopify Siparişlerini Getir",
        type="primary",
        use_container_width=True)

    if fetch_button:
        start_datetime = datetime.combine(
            start_date, datetime.min.time()).isoformat()
        end_datetime = datetime.combine(
            end_date, datetime.max.time()).isoformat()
        with st.spinner("Shopify'dan detaylı sipariş verileri çekiliyor..."):
            try:
                orders_result = shopify_api.get_orders_by_date_range(
                    start_datetime, end_datetime)
                st.session_state['shopify_orders_display'] = orders_result
                st.success(
                    f"✅ Başarıyla {
                        len(orders_result) if orders_result else 0} sipariş getirildi!")
            except Exception as e:
                st.error(
                    f"❌ Shopify siparişleri getirilirken hata oluştu: {
                        str(e)}")
    ...
    if not st.session_state['shopify_orders_display']:
        ...
        if financial_filter != "Tümü":
            orders = [o for o in orders if o.get(
                'displayFinancialStatus') == financial_filter]
        if fulfillment_filter != "Tümü":
            orders = [o for o in orders if o.get(
                'displayFulfillmentStatus') == fulfillment_filter]
        if customer_search:
            search_lower = customer_search.lower()
            orders = [
                o for o in orders if any(
                    [
                        search_lower in (
                            o.get('customer') or {}).get(
                            'firstName', '').lower(), search_lower in (
                            o.get('customer') or {}).get(
                            'lastName', '').lower(), search_lower in (
                                o.get('customer') or {}).get(
                                    'email', '').lower(), search_lower in (
                                        o.get('customer') or {}).get(
                                            'phone', '').lower()])]
        if order_search:
            orders = [
                o for o in orders if order_search.lower() in o.get(
                    'name', '').lower()]
        ...
        if orders:
            ...
            total_items = sum(sum(item.get('quantity', 0) for item in o.get(
                'lineItems', {}).get('nodes', [])) for o in orders)
            ...
            with status_cols[0]:
                ...
                for order in orders:
                    ...
                    financial_stats[status] = financial_stats.get(
                        status, 0) + 1
            ...
            with status_cols[1]:
                ...
                for order in orders:
                    status = order.get(
                        'displayFulfillmentStatus', 'Bilinmiyor')
                    fulfillment_stats[status] = fulfillment_stats.get(
                        status, 0) + 1
        ...
        if total_pages > 1:
            ...
            st.info(
                f"Sayfa {page + 1}/{total_pages} - Sipariş {start_idx + 1}-{end_idx}")
        else:
            page_orders = orders
        ...
        if view_mode == "Tablo Görünümü":
            ...
            for order in page_orders:
                ...
                if created_at:
                    try:
                        ...
                        date_str = created_at[:10] if len(
                            created_at) >= 10 else 'N/A'
                else:
                    date_str = 'N/A'

                table_data.append(
                    {
                        "Sipariş No": order.get(
                            'name',
                            'N/A'),
                        "Tarih": date_str,
            ...
            for order in page_orders:
                ...
                financial_status = order.get(
                    'displayFinancialStatus', 'Bilinmiyor')
                ...
                status_variant_map = {
                    'PAID': 'success',
                    'PENDING': 'warning',
                    'REFUNDED': 'neutral',  # neutral often better for refunded/cancelled
                ...
                with st.container(border=True):
                    ...
                    with cols[0]:
                        st.write(
                            f"**{order.get('name')}** - {customer_name or 'Misafir'}")
                        st.caption(f"📧 {customer.get('email', 'N/A')}")
                    with cols[1]:
                        st.write(f"**{total:.2f} {currency}**")
                    with cols[2]:
                        variant = status_variant_map.get(
                    ...
                    with cols[4]:
                        ...
                        if created_at:
                            try:
                                ...
                                st.caption(created_at[:10] if len(
                                    created_at) >= 10 else 'N/A')
                        else:
                            st.caption('N/A')
            ...
            for order in page_orders:
                ...
                if created_at:
                    try:
                        ...
                        date_str = created_at[:16] if len(
                            created_at) >= 16 else 'N/A'
                        date_display = created_at[:16] if len(
                            created_at) >= 16 else 'N/A'
                        order_date = None
                else:
                    date_str = 'N/A'
                ...
                with st.expander(f"🛍️ **{order.get('name')}** - {customer_name or 'Misafir'} ({date_str})", expanded=False):
                    ...
                    with info_cols[0]:
                        fin_badge = render_badge(
                            financial_status, status_variant_map.get(
                                financial_status, 'neutral'))
                        ship_badge = render_badge(
                            fulfillment_status, status_variant_map.get(
                                fulfillment_status, 'neutral'))

                        st.markdown(f"""
                        **📅 Sipariş Tarihi:** {date_display}
                        **💳 Ödeme Durumu:** {fin_badge}
                        **📦 Kargo Durumu:** {ship_badge}
                        """, unsafe_allow_html=True)

                        # Sipariş kimliği ve kaynağı
                        st.markdown(
                            f"**🆔 Sipariş ID:** `{order.get('id', 'N/A')}`")
                    ...
                    with detail_cols[0]:
                        ...
                        for item in order.get(
                            ...
                            line_items_data.append({
                                ...
                                # Toplam vergi dahil hesaplandı
                                "🧾 Toplam": (discounted_price * quantity) + tax_amount
                            })

                        df = pd.DataFrame(line_items_data)
                        st.dataframe(
                            df, use_container_width=True, hide_index=True, column_config={
                                "💵 Birim Fiyat": st.column_config.NumberColumn(
                                    format=f"%.2f {currency_code}"), "💰 İndirimli": st.column_config.NumberColumn(
                                    format=f"%.2f {currency_code}"), "📊 Vergi": st.column_config.NumberColumn(
                                    format=f"%.2f {currency_code}"), "🧾 Toplam": st.column_config.NumberColumn(
                                    format=f"%.2f {currency_code}")})
                    ...
                    with detail_cols[2]:
                        ...
                        if order.get('tags'):
                            ...
                            tags = order.get('tags', '').split(
                                ', ') if order.get('tags') else []
                            for tag in tags[:5]:  # İlk 5 etiketi göster
                                st.markdown(
                                    f"<span style='background-color:#e1f5fe; color:#01579b; padding: 2px 6px; border-radius: 10px; font-size: 12px; display: inline-block; margin: 2px;'>🏷️ {tag}</span>",
                                    unsafe_allow_html=True)
                        ...
                        if order.get('riskLevel'):
                            ...
                            variant = risk_colors.get(risk_level, 'neutral')
                        ...
                        if order.get('returns'):
                            st.markdown("### 🔄 İadeler")
                            st.info(
                                f"Bu siparişte {len(order.get('returns', []))} iade bulunmaktadır")
        ...
        if total_pages > 1 and view_mode != "Gelişmiş Tablo (Ag-Grid)":
            st.info(
                f"📄 Toplam {total_pages} sayfa • Gösterilen: {
                    len(page_orders)} sipariş • Toplam: {
                    len(orders)} sipariş")
