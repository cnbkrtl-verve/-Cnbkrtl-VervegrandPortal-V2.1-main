        ...
        if view_mode == "Tablo Görünümü":
            ...
            for order in page_orders:
                ...
                with st.container(border=True):
                    ...
                    with cols[2]:
                        st.badge(financial_status, color=status_colors.get(financial_status, 'gray'), icon="💳")
                    with cols[3]:
                        st.badge(fulfillment_status, color=status_colors.get(fulfillment_status, 'gray'), icon="📦")
                    with cols[4]:
                        # Güvenli tarih formatı
                        created_at = order.get('createdAt', '')
                        if created_at:
                ...
                with st.expander(f"🛍️ **{order.get('name')}** - {customer_name or 'Misafir'} ({date_str})", expanded=False):
                    ...
                    with detail_cols[2]:
                        ...
                        if order.get('riskLevel'):
                            ...
                            st.badge(risk_level, color=risk_colors.get(risk_level, 'gray'), icon="🛡️")
