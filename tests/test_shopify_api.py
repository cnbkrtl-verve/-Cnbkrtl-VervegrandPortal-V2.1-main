...
class TestShopifyAPIInit:
    ...
    def test_init_with_http_url(self):
        """✅ HTTP URL'i otomatik HTTPS'e çevrilmeli (Sadece https:// ile başlamıyorsa, http:// ise korunur)"""
        # Code logic: store_url if store_url.startswith('http') else f"https://{store_url.strip()}"
        # So http:// remains http://
...
class TestRateLimiter:
    ...
    def test_rate_limiter_initialization(self):
        ...
        assert api.max_requests_per_minute == 30  # Updated to 30
        assert api.burst_tokens == 5   # Updated to 5
        assert api.current_tokens == 5 # Updated to 5
