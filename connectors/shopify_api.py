...
class ShopifyAPI:
    ...
    def get_variant_ids_by_skus(self, skus: list, search_by_product_sku=False) -> dict:
        ...
        # ⚡ Bolt Optimization: Increased batch size for better performance while staying safe.
        # Cost Analysis:
        # - Max Query Cost = 1000
        # - Cost per product = 1 (node) + 50 (variants) = 51
        # - Safe limit = 1000 / 51 ~= 19 products
        # - Selected Batch Size = 10 (Cost ~510), safe buffer for other fields/complexity.
        batch_size = 10
        ...
        for i in range(0, len(sanitized_skus), batch_size):
            ...
            # Fetch as many products as the batch size to ensure we get all matches
            query_limit = batch_size + 5

            query = f"""
            query getProductsBySku($query: String!) {{
              products(first: {query_limit}, query: $query) {{
                edges {{
                  node {{
                    id
                    variants(first: 50) {{
            ...
            try:
                ...
                # ⚡ Bolt Optimization: Reduced static sleep time.
                # execute_graphql already handles throttling errors with backoff.
                # A small sleep is kept to be nice to the API in loops.
                time.sleep(0.5)
