...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        ...
        try:
            ...
            # Tek bir GraphQL sorgusu
            # Not: Query cost limit (1000) riski var ancak basit alanlar seçildiği için genelde sığar.
            combined_query = f"""
            query {{
              shop {{
                name
              ...
              ordersWeek: orders(first: 250, query: "created_at:>='{week_iso}'") {{
                edges {{
                  node {{
                    id
              ...
              ordersMonth: orders(first: 250, query: "created_at:>='{month_iso}'") {{
                edges {{
                  node {{
                    id
                    totalPriceSet {{ shopMoney {{ amount }} }}
            ...
            # Orders Today
            orders_today_data = result.get('ordersToday', {}).get('edges', [])
            stats['orders_today'] = len(orders_today_data)
            stats['revenue_today'] = sum(
                float(order['node'].get('totalPriceSet', {}).get('shopMoney', {}).get('amount', 0))
                for order in orders_today_data
            )
            stats['recent_orders'] = [order['node'] for order in orders_today_data[:5]]

            # Orders Week
            orders_week_data = result.get('ordersWeek', {}).get('edges', [])
            stats['orders_this_week'] = len(orders_week_data)
            stats['revenue_this_week'] = sum(
                float(order['node'].get('totalPriceSet', {}).get('shopMoney', {}).get('amount', 0))
                for order in orders_week_data
            )

            # Orders Month
            orders_month_data = result.get('ordersMonth', {}).get('edges', [])
            stats['orders_this_month'] = len(orders_month_data)
            stats['revenue_this_month'] = sum(
                float(order['node'].get('totalPriceSet', {}).get('shopMoney', {}).get('amount', 0))
                for order in orders_month_data
            )
