...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        ...
        try:
            ...
            # ⚡ BOLT OPTIMIZATION: Combine 6 GraphQL queries into 1
            # Combined Query to fetch all dashboard stats in one go
            # Using aliases for different order queries
            combined_query = """
            ...
            result = self.execute_graphql(combined_query, variables)
            if not result:
                return stats
            ...
            # Parse Orders Today
            if orders_today_data := result.get('ordersToday'):
            ...
            # Parse Orders Week
            if orders_week_data := result.get('ordersWeek'):
            ...
            # Parse Orders Month
            if orders_month_data := result.get('ordersMonth'):
