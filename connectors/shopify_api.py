...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        ...
        try:
            # Zaman pencerelerini hesapla (Sunucu saati - UTC varsayılır)
            today = datetime.now()
            ...
            # TEK SORGULAMA: Shop, Products, Orders (Month+)
            query = """
            query getDashboardStats($month_start: String!) {
              shop {
                name
                email
                primaryDomain { host }
              ...
              orders(first: 250, query: $month_start, sortKey: CREATED_AT, reverse: true) {
                edges {
                  node {
                    id
                    name
            ...
            variables = {
                "month_start": f"created_at:>='{month_start_iso}'"
            }
            ...
            has_more = products_data.get('pageInfo', {}).get('hasNextPage', False)
            if has_more:
                stats['products_count_note'] = f"{stats['products_count']}+ (daha fazla ürün var)"
            
            # 3. Sipariş İstatistikleri
            all_orders = [edge['node'] for edge in data.get('orders', {}).get('edges', [])]
