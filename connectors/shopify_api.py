...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        """Dashboard için detaylı istatistikleri getir - Optimize Edilmiş Versiyon"""
        stats = {
            'shop_info': {},
            'orders_today': 0,
            'orders_this_week': 0,
        ...
        try:
            # 1. Metadata Query: Shop ve Products Count'u birleştir
            metadata_query = """
            query {
              shop {
                name
            ...
            # 2. Orders Query: Tek seferde en geniş tarih aralığını çek
            ...
            # En eski tarihi bul (genelde month_start, ama ayın ilk günleri için week_start bir önceki aya sarkabilir)
            min_date_iso = min(week_iso, month_iso)

            # Tek sorgu ile siparişleri çek (En çok 250 sipariş, en yeniden eskiye)
            orders_query = f"""
            query {{
            ...
            if orders_result:
                ...
                # Python tarafında filtrele ve hesapla
                today_orders = []

                for order in all_orders:
                    created_at = order.get('createdAt')  # ISO String: 2023-10-27T10:00:00Z
                    ...
                    if created_at >= month_iso:
                        ...
                        stats['revenue_this_month'] += float(order.get('totalPriceSet', {}).get('shopMoney', {}).get('amount', 0))

                # Son siparişler (Sadece bugünün siparişlerini göster)
