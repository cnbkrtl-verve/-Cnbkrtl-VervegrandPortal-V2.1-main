...
class ShopifyAPI:
    ...
    def get_variant_ids_by_skus(self, skus: list, search_by_product_sku=False) -> dict:
        """
        Optimized version: Batches requests and relies on built-in rate limiter.
        """
        if not skus: return {}
        ...
        for i in range(0, len(sanitized_skus), batch_size):
            ...
            try:
                logging.info(f"SKU batch {i//batch_size+1}/{len(range(0, len(sanitized_skus), batch_size)) + 1} işleniyor: {sku_chunk}")
                ...
                # Removed hardcoded sleep; relying on _rate_limit_wait in execute_graphql
