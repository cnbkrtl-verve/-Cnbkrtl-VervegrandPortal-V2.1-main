...
class ShopifyAPI:
    ...
    def __init__(self, store_url: str, access_token: str, api_version: str = '2024-10'): # api_version parametresi burada ekli olmalı
        ...
        # ⚡ OPTIMIZATION (Bolt): HTTPS zorlaması (Security fix + Test compliance)
        # HTTP girilse bile HTTPS'e çevir
        if store_url.startswith('http:'):
             self.store_url = store_url.replace('http:', 'https:')
        else:
             self.store_url = store_url if store_url.startswith('https') else f"https://{store_url.strip()}"
    ...
    def get_variant_ids_by_skus(self, skus: list, search_by_product_sku=False) -> dict:
        ...
        # ⚡ OPTIMIZATION (Bolt): Batch boyutunu 20'ye çıkarıldı ve gereksiz bekleme kaldırıldı.
        # execute_graphql zaten rate limit kontrolü yapıyor.
        batch_size = 20
        ...
        for i in range(0, len(sanitized_skus), batch_size):
            ...
            # first: 20 olarak güncellendi (batch size ile uyumlu)
            query = """
            ...
            try:
                logging.info(f"SKU batch {i//batch_size+1}/{len(range(0, len(sanitized_skus), batch_size))} işleniyor: {len(sku_chunk)} adet SKU")
                ...
                # Gereksiz time.sleep(3) kaldırıldı. execute_graphql içindeki rate limiter yeterli.
            
            except Exception as e:
                logging.error(f"SKU grubu {i//batch_size+1} için varyant ID'leri alınırken hata: {e}")
                # Hata durumunda beklemek mantıklı olabilir ama execute_graphql zaten retry yapıyor.
                # Yine de üst katmana hatayı bildirmeden önce kısa bir bekleme kalabilir veya kaldırılabilir.
                # execute_graphql'in başarısız olması durumunda buraya düşeriz.
                time.sleep(1)
                raise e
