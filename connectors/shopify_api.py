...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        """Dashboard için detaylı istatistikleri tek bir sorguda getir (Optimize edildi ⚡)"""
        stats = {
            'shop_info': {},
            'orders_today': 0,
            'orders_this_week': 0,
        ...
        try:
            ...
            # ⚡ OPTIMIZASYON: Tüm sorgular tek bir GraphQL isteğinde birleştirildi
            # Aliases kullanılarak farklı tarih aralıkları ve veri tipleri aynı anda çekiliyor
            combined_query = f"""
            query DashboardStats {{
              ...
              orders_today: orders(first: 50, query: "created_at:>='{today_iso}' AND created_at:<'{tomorrow_iso}'") {{
                edges {{
                  node {{
                    id
                    name
              ...
              orders_week: orders(first: 250, query: "created_at:>='{week_iso}'") {{
                edges {{
                  node {{
                    id
              ...
              orders_month: orders(first: 250, query: "created_at:>='{month_iso}'") {{
                edges {{
                  node {{
                    id
                    totalPriceSet {{ shopMoney {{ amount }} }}
            ...
            result = self.execute_graphql(combined_query)
            if not result:
                return stats
            ...
            # 2. Ürün Sayısı
            if products_data := result.get('products'):
            ...
            # 3. Müşteri Sayısı (Placeholder)
            # customers_data = result.get('customers')

            # 4. Bugünün Siparişleri
            if orders_today_data := result.get('orders_today'):
                today_orders = orders_today_data.get('edges', [])
            ...
            # 5. Bu Haftanın Siparişleri
            if orders_week_data := result.get('orders_week'):
                week_orders = orders_week_data.get('edges', [])
            ...
            # 6. Bu Ayın Siparişleri
            if orders_month_data := result.get('orders_month'):
                month_orders = orders_month_data.get('edges', [])
