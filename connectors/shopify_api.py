...
class ShopifyAPI:
    ...
    def get_dashboard_stats(self):
        """Dashboard için detaylı istatistikleri getir - OPTİMİZE EDİLMİŞ VERSİYON"""
        stats = {
            'shop_info': {},
            'orders_today': 0,
            'orders_this_week': 0,
        ...
        try:
            # 1. Ürün sayısı (Query Cost'u dengelemek için ayrı tutuldu)
            products_query = """
            query { 
              products(first: 250) { 
            ...
            if products_result:
                ...
                if products_result.get('products', {}).get('pageInfo', {}).get('hasNextPage', False):
                    stats['products_count_note'] = f"{stats['products_count']}+ (daha fazla ürün var)"

            # 2. Consolidated Query: Shop + Recent Orders (Detailed) + Stats Orders (Lightweight)
            # - Shop: Mağaza bilgileri
            # - recentOrders: Son 10 sipariş (Detaylı, listeleme için)
            # - statsOrders: Son 250 sipariş (Özet, istatistik hesaplama için)
            consolidated_query = """
            query {
              shop {
                name
              ...
              statsOrders: orders(first: 250, sortKey: CREATED_AT, reverse: true) {
                edges {
                  node {
                    createdAt
            ...
            data = self.execute_graphql(consolidated_query)
            
            # A. Shop Info
            if shop_data := data.get('shop'):
                stats['shop_info'] = shop_data
            
            # B. Recent Orders
            if recent_orders_data := data.get('recentOrders', {}).get('edges', []):
                stats['recent_orders'] = [edge['node'] for edge in recent_orders_data]
            
            # C. Stats Calculations (Today, Week, Month)
            if stats_orders_data := data.get('statsOrders', {}).get('edges', []):
                now = datetime.now()
                today_start = now.replace(hour=0, minute=0, second=0, microsecond=0)
                week_start = today_start - timedelta(days=today_start.weekday())
                month_start = today_start.replace(day=1)

                for edge in stats_orders_data:
                    node = edge['node']
                    created_at_str = node.get('createdAt', '').replace('Z', '+00:00')
                    try:
                        created_at = datetime.fromisoformat(created_at_str)
                        # UTC'den yerel saate çevir (Naive)
                        created_at_local = created_at.astimezone(None).replace(tzinfo=None)

                        amount = float(node.get('totalPriceSet', {}).get('shopMoney', {}).get('amount', 0))

                        if created_at_local >= today_start:
                            stats['orders_today'] += 1
                            stats['revenue_today'] += amount

                        if created_at_local >= week_start:
                            stats['orders_this_week'] += 1
                            stats['revenue_this_week'] += amount

                        if created_at_local >= month_start:
